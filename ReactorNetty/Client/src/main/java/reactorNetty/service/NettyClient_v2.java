...
@Slf4j
public class NettyClient_v2 implements NettyClient<byte[]> {

    private final ServerConfig serverConfig;

    private Connection connection;

    public NettyClient_v2(ServerConfig serverConfig) {
    ...
    public void sendMessage(byte[] msg) {
        if (isConnected()) {
            connection.outbound()
                    .sendObject(Mono.just(Unpooled.wrappedBuffer(msg)))
                    .then()
                    .doOnTerminate(() -> log.info("Sent : {}", new String(msg, StandardCharsets.UTF_8)))
                    .then()
